/* eslint-disable */

// @ts-nocheck

// This file was automatically generated by @caffeinelabs/bindgen@0.1.0.
// You should NOT make any changes in this file as it will be overwritten.
// Additionally, you should also exclude this file from your linter and/or formatter to prevent it from being checked or modified.

import { IDL } from '@icp-sdk/core/candid';

export const HomeType = IDL.Variant({
  'PG' : IDL.Null,
  'OneBHK' : IDL.Null,
  'TwoBHK' : IDL.Null,
  'FourBHKPlus' : IDL.Null,
  'ThreeBHK' : IDL.Null,
});
export const Time = IDL.Int;
export const WaitlistEntry = IDL.Record({
  'preferredStartDate' : IDL.Text,
  'homeType' : HomeType,
  'city' : IDL.Text,
  'name' : IDL.Text,
  'elderly' : IDL.Nat,
  'email' : IDL.Text,
  'children' : IDL.Nat,
  'squareFootage' : IDL.Nat,
  'timestamp' : Time,
  'adults' : IDL.Nat,
  'phone' : IDL.Text,
});

export const idlService = IDL.Service({
  'addCalculatorQuery' : IDL.Func(
      [HomeType, IDL.Nat, IDL.Nat, IDL.Nat, IDL.Nat, IDL.Nat, IDL.Nat],
      [],
      [],
    ),
  'addWaitlistEntry' : IDL.Func(
      [
        IDL.Text,
        IDL.Text,
        IDL.Text,
        IDL.Text,
        HomeType,
        IDL.Nat,
        IDL.Nat,
        IDL.Nat,
        IDL.Nat,
        IDL.Text,
      ],
      [],
      [],
    ),
  'getAllWaitlistEntries' : IDL.Func([], [IDL.Vec(WaitlistEntry)], ['query']),
  'getWaitlistCount' : IDL.Func([], [IDL.Nat], ['query']),
});

export const idlInitArgs = [];

export const idlFactory = ({ IDL }) => {
  const HomeType = IDL.Variant({
    'PG' : IDL.Null,
    'OneBHK' : IDL.Null,
    'TwoBHK' : IDL.Null,
    'FourBHKPlus' : IDL.Null,
    'ThreeBHK' : IDL.Null,
  });
  const Time = IDL.Int;
  const WaitlistEntry = IDL.Record({
    'preferredStartDate' : IDL.Text,
    'homeType' : HomeType,
    'city' : IDL.Text,
    'name' : IDL.Text,
    'elderly' : IDL.Nat,
    'email' : IDL.Text,
    'children' : IDL.Nat,
    'squareFootage' : IDL.Nat,
    'timestamp' : Time,
    'adults' : IDL.Nat,
    'phone' : IDL.Text,
  });
  
  return IDL.Service({
    'addCalculatorQuery' : IDL.Func(
        [HomeType, IDL.Nat, IDL.Nat, IDL.Nat, IDL.Nat, IDL.Nat, IDL.Nat],
        [],
        [],
      ),
    'addWaitlistEntry' : IDL.Func(
        [
          IDL.Text,
          IDL.Text,
          IDL.Text,
          IDL.Text,
          HomeType,
          IDL.Nat,
          IDL.Nat,
          IDL.Nat,
          IDL.Nat,
          IDL.Text,
        ],
        [],
        [],
      ),
    'getAllWaitlistEntries' : IDL.Func([], [IDL.Vec(WaitlistEntry)], ['query']),
    'getWaitlistCount' : IDL.Func([], [IDL.Nat], ['query']),
  });
};

export const init = ({ IDL }) => { return []; };